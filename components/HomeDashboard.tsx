...
import { MENTORS, WORKSHOPS, COURSES, PRO_FEATURES } from '../constants';
import MentorCard from './MentorCard';
import Hero from './Hero';
import { Mentor, Workshop, Course, ProFeature } from '../types';
    ...
    <div className="space-y-16 sm:space-y-24 animate-slide-in-fade">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-12">
        ...
        <div className="lg:col-span-2 lg:border-l lg:border-slate-800 lg:pl-8">
            {/* AI Suggested Courses Section */}
            <section>
                <h2 className="text-3xl font-bold text-white mb-1 flex items-center">
                   <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-7 h-7 mr-3 text-purple-400"><path d="M17 18a2 2 0 0 0-2-2H9a2 2 0 0 0-2 2"></path><rect width="18" height="18" x="3" y="4" rx="2"></rect><circle cx="12" cy="10" r="2"></circle><line x1="8" x2="8" y1="2" y2="4"></line><line x1="16" x2="16" y1="2" y2="4"></line></svg>
                   AI Suggests
                   <span className="ml-3 text-sm font-semibold bg-yellow-400/20 text-yellow-300 px-2.5 py-1 rounded-full">PRO</span>
                </h2>
                <p className="text-sm text-muted-gray mb-6">Powered by Gemini</p>
                <div className="space-y-4">
                {COURSES.map((course) => (
                    <div 
                      key={course.title}
                      onClick={() => handleSelectCourse(course)}
                      className="cursor-pointer block bg-slate-800/30 backdrop-blur-sm border border-slate-700 rounded-lg p-4 hover:border-aqua-green transition-colors hover:bg-slate-800/50"
                    >
                        <h3 className="font-semibold text-white">{course.title}</h3>
                        <p className="text-xs text-aqua-green font-medium mb-2">{course.platform}</p>
                        <p className="text-sm text-muted-gray">{course.description}</p>
                    </div>
                ))}
                </div>
            </section>
